...
for (var index in bucketKeys) {
	dbs[config.get('couchbase.buckets.' + bucketKeys[index] + '.name') + ',' + config.get('couchbase.buckets.' + bucketKeys[index] + '.password')] = null;
}

// To-do when connection is no longer active or disconnected start new
// connection
var conn = function (bucketName, password, cb) {
	if (dbs[bucketName]) {
		return cb(null, dbs[bucketName]);
	}

	dbs[bucketName] = cluster.openBucket(bucketName, password, function (error) {
		if (error) {
...
var commandWithConnection = function (cmd) {
	return function () {
		...
		// the callback
		conn(bucketName.name, bucketName.password, function (err, conn) {
			if (err) {
				return cb(err);
			}
